 ...
 *
 * @param username The username for the TURN server.
 *
 ...
 *
 * While you can use the <[OTSession forceDisconnect:error:]> method to
 * terminate your own connection, calling the <[OTSession disconnect:]> method is simpler. An
 * error is returned if the client's role does not include permissions
 * required to force other users to disconnect. You define a client's role when
 * you create the user token. When a connection is terminated using this
 * method , <[OTSessionDelegate sessionDidDisconnect:]>,
 * <[OTSessionDelegate session:connectionDestroyed:]>
 * and <[OTPublisherKitDelegate publisher:streamDestroyed:]> messages are
 * dispatched in the same way as they would be if the connection had
 * terminated itself using <[OTSession disconnect:]>
 *
 * @param connection The OTConnection to disconnect.
 * @param error If force disconnect fails, this value is set to an OTError
 * object. The OTSessionErrorCode enum (in OTError.h) includes
 * OTSessionUnableToForceDisconnect for these errors.
 */
- (void)forceDisconnect:(OTConnection* _Nullable)connection
                  error:(OTError* _Nullable* _Nullable)error;
...
- (void)setRtcStatsReportFilePath:(nonnull NSString*)path
                      error:(OTError* _Nullable* _Nullable)error
__attribute__((deprecated("This method is deprecated and will be removed in future minor releases. Please use the [OTPublisher getRtcStatsReport] and [OTSubscriber getRtcStatsReport] methods instead.")));
...
/**
 * Whether the client can force disconnect other connections in the session
 * (`YES`) or not (`NO`).
 *
 * See <[OTSession forceDisconnect:error:]>
 */
@property (readonly) BOOL canForceDisconnect;

- (nonnull instancetype)initWithCanPublish:(BOOL)publish
                              canSubscribe:(BOOL)subscribe
                              canForceMute:(BOOL)forceMute
                        canForceDisconnect:(BOOL)forceDisconnect;
