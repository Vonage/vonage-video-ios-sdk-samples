 ...
 */
@interface OTSenderStats : NSObject

/**
 * The maximum bitrate that can be estimated for the sender connection, in bps (bits
 * per second).
 */
@property (readonly) int64_t maxBitrate;

/**
 * The current bandwidth estimation for the sender connection, , in bps (bits per
 * second).
 */
@property (readonly) int64_t currentBitrate;

- (nonnull instancetype)initWithMaxBitrate:(int64_t)maxBitrate
                            currentBitrate:(int64_t)currentBitrate;
 ...
 * This property is optional and may be nil if data is not available.
 * Due to network latency, sender statistics may not be
 * immediately available after the initial stats request.
 */
...
- (nonnull instancetype)initWithPacketsLost:(uint64_t)packetsLost
                            packetsReceived:(uint64_t)packetsReceived
                              bytesReceived:(uint64_t)bytesReceived
                                  timestamp:(double)timestamp
                                senderStats:(OTSenderStats * _Nullable)senderStats;
@end
