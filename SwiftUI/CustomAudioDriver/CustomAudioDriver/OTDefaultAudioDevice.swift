...
public class OTDefaultAudioDevice: NSObject, OTAudioDevice {
    ...
    private func teardownAudio() {
        ...
        try? session.setMode(previousMode)
        try? session.setPreferredSampleRate(previousPrefSampleRate)
        try? session.setPreferredInputNumberOfChannels(previousPrefChannels)
    ...
    private func freeBuffer() {
        if let bufferList = bufferList {
            let pointer = bufferList.pointee.mBuffers.mData
            pointer?.deallocate()
    ...
    private func setupAudioUnit(_ unit: inout AudioUnit?, isPlayout: Bool) -> Bool {
        ...
        guard AudioComponentInstanceNew(comp, &audioUnit) == noErr else { return false }
        ...
        if isPlayout == false {
            ...
            // CALLBACK
            var auRenderCallback = AURenderCallbackStruct(
            ...
            AudioUnitSetProperty(audioUnit!,
                                 ...
                                 1,
                                 &auRenderCallback,
                                 UInt32(MemoryLayout.size(ofValue: auRenderCallback)))
    ...
    private func handleRecordingCallback(flags: UnsafeMutablePointer<AudioUnitRenderActionFlags>?,
        ...
        guard let unit = recordingUnit, let ts else { return noErr }
